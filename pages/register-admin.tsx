...
const RegisterAdmin = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        const response = await fetch(`${constantUrlApiEndpoint}/register-admin`, {
          method: "POST",
        ...
        setSuccessMessage("Registro de administrador exitoso. Redirigiendo al login...");
        setTimeout(() => {
          router.push("/dashboard");
        }, 500);
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : "Error desconocido";
        console.error("Error al registrar administrador:", message);
        setError(message);
      } finally {
        setLoading(false);
      }
